...
class KVCacheModelFT():
    ...
    def _forward_with_kvcache(self, input_ids : torch.Tensor, use_debug = True) -> torch.Tensor:
        if self._start_pos is None:
            assert self._prob_history is None, f"{self._prob_history.shape}"
            # the first forward (prefill) returns the prompt's logits
            outputs = self._model(input_ids, start_pos = 0)
            self._prob_history = outputs.clone()
            for i in range(self._prob_history.shape[-2]):   
                self._prob_history[:, i, :] = norm_logits(self._prob_history[:, i, :], self._temperature, self._top_k, self._top_p)
            self._start_pos = outputs.shape[1]
            last_q = self._prob_history[:, -1, :]
        else:
            # return the last token's logits
            cached_len = self._start_pos
                
            last_input_id = input_ids[:, cached_len:]
            if last_input_id.dim() == 1:
                last_input_id = torch.unsqueeze(last_input_id, 0)
            # if last_input_id.shape[1] > 1:
            #     import code; code.interact('line 147', local=dict(globals(), **locals()))
            
            outputs = self._model(last_input_id, start_pos=cached_len)

            not_cached_q = outputs.clone()
            if not_cached_q.dim() == 2:
                not_cached_q = torch.unsqueeze(not_cached_q, 0)
            ...
            last_q = not_cached_q[:, -1, :]
            self._start_pos += not_cached_q.shape[-2]
    ...
    @torch.no_grad()
    def rollback(self, end_pos : int):
        assert self._start_pos
        end_pos = min(self._start_pos, end_pos)
