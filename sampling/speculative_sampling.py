...
from sampling.kvcache_model import KVCacheModel, KVCacheModelFT
...
def speculative_sampling(prefix : torch.Tensor, approx_model : torch.nn.Module, target_model : torch.nn.Module, 
                         ...
                         temperature : float = 1, top_k : int = 0, top_p : float = 0, verbose : bool = False, random_seed : int = None, ft=False) -> torch.Tensor:
    """
    ...
    # import code; code.interact('SP', local=dict(globals(), **locals()))

    target_device = target_model.device
    approx_device = approx_model.device
    
    if ft:
        approx_model_cache = KVCacheModelFT(approx_model, temperature, top_k, top_p)
        target_model_cache = KVCacheModelFT(target_model, temperature, top_k, top_p)
    else:
    ...
    while prefix.shape[1] < T:
        ...
        # import code; code.interact(f'Start Loop. prefix_len : {prefix_len}', local=dict(globals(), **locals()))

        x = approx_model_cache.generate(prefix.to(approx_device), gamma)
        ...
        for i in range(gamma):
            ...
            j = int(x[:, prefix_len + i])
            
            if r > (target_model_cache._prob_history[:, prefix_len + i - 1, j]) / (approx_model_cache._prob_history[:, prefix_len + i - 1, j].to(target_device)):
                # reject
                n = prefix_len + i - 1
                break
    ...
    return prefix, prefix.shape[-1] - seq_len - target_sample_count, accepted_count, resample_count
